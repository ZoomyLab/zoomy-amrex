...
ChorinAmr::ChorinAmr()
{
    ...
    { auto tags = ModelPred::get_boundary_tags();  // structured face convention:
      auto tidx=[&](const char* nm){ for(int t=0;t<(int)tags.size();++t) if(tags[t]==std::string(nm)) return t; return -1; };
      bc.side_tag[0]=tidx("West"); bc.side_tag[1]=tidx("East");   // x-lo / x-hi
      bc.side_tag[2]=tidx("South"); bc.side_tag[3]=tidx("North"); // y-lo / y-hi
      bc.params = pPred;
